...
##'
##' modified from refund to provide orthogonal FPC estimates
##'
...
my.fpca.sc <- function (Y = NULL, ydata = NULL, Y.pred = NULL, argvals = NULL,
...
{
	library(mgcv)
	stopifnot((!is.null(Y) && is.null(ydata)) || (is.null(Y) &&
																									!is.null(ydata)))
	...
	if (center) {
		if (random.int) {
			ri_data <- data.frame(y = as.vector(Y), d.vec = d.vec,
														id = factor(id))
			gam0 = gamm4(y ~ s(d.vec, k = nbasis), random = ~(1 |
																													id), data = ri_data)$gam
			rm(ri_data)
		}
	...
	if (cov.est.method == 2) {
		...
		if (!useSymm) {
			...
			npc.0 = matrix(predict(gam(as.vector(G.0) ~ te(row.vec,
																										 col.vec, k = nbasis), weights = as.vector(cov.count)),
			...
			usecov.count[2, 1] <- usecov.count[ncol(G.0), ncol(G.0) -
																				 	1] <- 0
			...
			npc.0 <- matrix(NA, D, D)
			spred <- rep(argvals, each = D)[upper.tri(npc.0,
																								diag = TRUE)]
			tpred <- rep(argvals, times = D)[upper.tri(npc.0,
																								 diag = TRUE)]
		...
		npc.0 = matrix(predict(gam(G.0.vec ~ te(row.vec, col.vec,
																						k = nbasis)), newdata = data.frame(row.vec = row.vec.pred,
	...
	npc = ifelse(is.null(npc), min(which(cumsum(evalues)/sum(evalues) >
																			 	pve)), npc)
	efunctions = matrix(Winvsqrt %*% eigen(V, symmetric = TRUE)$vectors[,
	...
	for (i.subj in 1:I.pred) {
		...
		Yhat[i.subj, ] = t(as.matrix(mu)) + scores[i.subj, ] %*%
			t(efunctions)
		if (var) {
			VarMats[[i.subj]] = sigma2 * Z %*% ZtZ_sD.inv %*%
				t(Z)
			...
			if (simul & sigma2 != 0) {
				...
				crit.val[i.subj] = quantile(apply(abs(norm.samp),
																					1, max), sim.alpha)
			}
		}
	}
	ret.objects = c("Yhat", "Y", "scores", "mu", "efunctions",
									"evalues", "npc", "argvals")
	if (var) {
